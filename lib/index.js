...
exports.compile = function(input, options, callback) {
  ...
  if ('string' === typeof options) {
    args = options.split(/\s+/);
    if (args.length && !args[0].length) args.shift();
    if (args.length && !args[args.length-1].length) args.pop();
    if (args.indexOf('-jar') == -1) {
      args.unshift(JAR_PATH);
      args.unshift('-jar');
    }
  }
  else {
